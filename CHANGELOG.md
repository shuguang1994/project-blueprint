...
### Added
- **外部评估复核**：内部复核报告已完成（含门禁脚本实跑、20+ 项数字口径核对、4 篇论文与 3 个外部站点独立复核；**文件名与结论不在公开仓描述**）
- 补登记 2 份内部文档入库（**不随公开仓发布**；此前仅为本地未跟踪文件，索引与实际文件不一致）
- **机制可发表性交叉验证（v2.0）**：内部报告由 v1.0 的「零件级对撞」改为**集成层口径**（8 环 × 6 个候选系统），并纳入元规则层；结合 `gate-audit` 首份实跑数据划定**样本使用边界**（区分「回溯提炼源」与「正向应用样本」，显式排除循环论证）与后续论文方向。v1.0 的零件级结论已废弃
