...
Project Blueprint 是一个 **AI Agent 技能包**，为新项目一键建立完整的开发规范体系。它不是一个静态模板——它是一个**自主发现引擎**：扫描你的项目文件，智能推断技术栈，从 95 个组件条目的知识库实时拼装定制化的 AGENTS.md、文档骨架、CI/CD 和测试制度。
...
| **业务类型推断**       | 两层启发式（结构特征 + 配置特征），覆盖 12 种业务类型                                    |
| **动态 AGENTS.md** | 从 95 个组件条目的知识库实时拼装，非固定模板                                             |
...
| **业务类型感知**        | 12 种业务类型推断，驱动不同的文档结构生成                                          | 无竞品推断项目类型                                       |
...
| **7 语言 15 框架知识库** | 95 个组件条目含 Commands + Conventions + CI，中文为第一语言                     | 竞品最多覆盖 JS/TS 生态                                 |
...
- **原生中文** — 7 语言 15 框架 95 个组件条目，中文为第一语言
...
Step 2: 规则引擎从 95 个组件条目的知识库拼装 AGENTS.md
    ↓ （未知栈 → 联网搜索回退）
    ↓ （多子项目 → 根 AGENTS.md + 各包级 AGENTS.md）
Step 3: 按业务类型动态创建文档骨架（12 种类型）+ MCP 工具推荐（B-05）
    ↓
